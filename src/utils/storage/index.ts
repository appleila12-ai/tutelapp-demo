...
import { chiavePerComune } from "@/src/config/comune";
...
export class Storage extends StorageBase {
  ...
  async getItem<Fallback extends StorageItemValue>(
    ...
    try {
      const raw = await AsyncStorage.getItem(chiavePerComune(key));
  ...
  async setItem<Value extends StorageItemValue>(
    ...
    try {
      await AsyncStorage.setItem(chiavePerComune(key), JSON.stringify(value));
      return true;
    } catch (e) {
  ...
  async removeItem(key: string): Promise<boolean> {
    try {
      await AsyncStorage.removeItem(chiavePerComune(key));
      return true;
    } catch (e) {
